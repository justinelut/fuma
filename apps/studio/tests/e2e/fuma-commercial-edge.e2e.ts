...
test.describe('FUMA-048–062 commercial edge browser acceptance',()=>{
 ...
 test('public web fails an unknown host closed and does not render a default tenant',async({request})=>{const response=await request.get(`${PUBLIC}/__fuma/edge-probe`,{headers:{host:'unknown.trimly.co.ke'}});expect([404,421]).toContain(response.status());expect(await response.text()).not.toContain('default site')})
