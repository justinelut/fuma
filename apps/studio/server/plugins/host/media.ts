...
export type MediaWorkerRequester = (
  pluginId: string,
  message: Extract<MainToWorkerMessage, { kind: 'run-media-adapter-call' }>,
  expectedKind: 'media-adapter-call-result',
) => Promise<Extract<WorkerToMainMessage, { kind: 'media-adapter-call-result' }>>
...
async function runMediaAdapterCallInWorker(
  ...
  args: unknown[],
  requestWorker: MediaWorkerRequester,
): Promise<unknown> {
...
}, requestWorker: MediaWorkerRequester = requestFromWorker): MediaStorageAdapter {
  const call = (
    method: 'beginWrite' | 'finalizeWrite' | 'abortWrite' | 'delete' | 'getReadUrl' | 'verify',
    methodArgs: unknown[],
  ): Promise<unknown> => runMediaAdapterCallInWorker(
    args.pluginId,
    args.adapterId,
    method,
    methodArgs,
