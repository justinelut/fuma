...
function requestHasSessionCookie(req: Request | undefined, cookieName = SESSION_COOKIE_NAME): boolean {
  if (!req) return false
  ...
  for (const part of cookie.split(';')) {
    ...
    if (eq < 0) continue
    if (part.slice(0, eq).trim() === cookieName) return true
  }
  return false
}
...
const HOSTED_AUTH_FLAG_SCRIPT = `
    <script>window.__fumaHostedStaffAuth = 1;</script>`
function injectAuthenticatedHints(
  html: string,
  staticDir: string,
  hosted: boolean,
): string {
  const preloadHints = buildAuthenticatedShellPreloadHints(staticDir)
  const authScripts = hosted
    ? `${HOSTED_AUTH_FLAG_SCRIPT}\n${AUTHED_FLAG_SCRIPT}`
    : `${AUTHED_FLAG_SCRIPT}\n${BOOT_API_KICKOFF}`
...
function injectLoginSkeleton(html: string, hosted: boolean): string {
  // 1. Inject the skeleton CSS right after the loader CSS block so the
  //    critical styles are sent in the first response packet. The hosted
  //    shell is selected explicitly by the server and must not preflight the
  //    self-hosted setup/session endpoints.
  const styleTag = `<style data-initial-login>${LOGIN_SKELETON_STYLES}</style>`
  const bootScript = hosted ? HOSTED_AUTH_FLAG_SCRIPT : BOOT_API_KICKOFF
  ...
  if (!next.includes('data-initial-login')) {
    next = next.replace('</head>', `  ${styleTag}${bootScript}\n  </head>`)
  }
...
export interface AdminAppServeOptions {
  hostedCookieName: string
}

export async function serveAdminApp(
  staticDir: string,
  req?: Request,
  options?: AdminAppServeOptions,
): Promise<Response | null> {
  ...
  const transformed = requestHasSessionCookie(req, options?.hostedCookieName)
    ? injectAuthenticatedHints(html, staticDir, hosted)
    : injectLoginSkeleton(html, hosted)
