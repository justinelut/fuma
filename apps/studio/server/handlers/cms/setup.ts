...
export async function handleSetupRoutes(req: Request, db: DbClient): Promise<Response | null> {
  ...
  if (url.pathname === `${CMS_API_PREFIX}/setup/status`) {
    ...
    return jsonResponse(await getSetupStatus(db, SELF_HOST_SITE_ID))
  }

  if (url.pathname === `${CMS_API_PREFIX}/public-site`) {
    if (req.method !== 'GET') return methodNotAllowed()
    return jsonResponse(await loadPublicSiteIdentity(db, SELF_HOST_SITE_ID))
  }

  if (url.pathname === `${CMS_API_PREFIX}/setup`) {
    if (req.method !== 'POST') return methodNotAllowed()
    const status = await getSetupStatus(db, SELF_HOST_SITE_ID)
    ...
    return await db.transaction(async (tx) => {
      await createSite(tx, SELF_HOST_SITE_ID, siteName, {})
 ...
 * without auth.
 */
async function loadPublicSiteIdentity(db: DbClient, siteId: string): Promise<PublicSiteIdentity> {
