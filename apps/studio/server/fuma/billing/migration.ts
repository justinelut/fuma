...
export const platformBillingReconciliationMigrationCandidate = Object.freeze({
  ...
  sql: `
    ...
    alter table fuma_contract_candidates
      drop constraint fuma_contract_candidates_state_check;
    alter table fuma_contract_candidates
      add constraint fuma_contract_candidates_state_check check (
        state in ('awaiting-payment','partially-paid','active','paid-transfer-pending')
      );
