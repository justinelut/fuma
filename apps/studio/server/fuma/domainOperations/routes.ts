...
async function body<T extends TSchema>(input:FumaScopedRouteHandlerInput,schema:T){const value=await input.request.json().catch(()=>null);const parsed=safeParseValue(schema,value);if(!parsed.ok)throw new DomainOperationError('invalid-contract','Request contract is invalid.');return parsed.value}
export function createDomainOperationsScopedRoutes(service:DomainOperationsService):readonly FumaScopedRouteDeclaration[]{
  ...
  return Object.freeze([
    route('GET','/settings/domains/:domainId/operations','site.settings.read',async(input)=>{const value=await service.exactSettings(scope(input),input.params.domainId);return value?response(CustomerDnsSettingsSchema,value):response(ErrorSchema,{error:'Domain settings were not found.'},404)}),
    route('POST','/settings/domains/customer-dns/onboard','site.settings.write',async(input)=>{const value=await body(input,OnboardSchema);return response(CustomerDnsSettingsSchema,await service.onboardCustomerDns(scope(input),value.domainId,value.prevalidation,value.capability),201)}),
    route('POST','/settings/domains/:domainId/automation','site.settings.write',async(input)=>response(CustomerDnsSettingsSchema,await service.attachCustomerAutomation(scope(input),input.params.domainId,await body(input,CustomerAutomationCommandSchema)))),
    ...
    route('POST','/settings/domains/detach','site.settings.write',async(input)=>{const value=await body(input,DetachSchema);return response(CustomerDnsSettingsSchema,await service.safeDetach(scope(input),value.domainId,value.operationId))}),
  ])
}
