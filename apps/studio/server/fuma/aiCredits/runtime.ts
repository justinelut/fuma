...
export function createHostedAiCreditRuntime(input: Readonly<{ db: DbClient; catalog: Pick<AiCatalogService, 'quote'>; cipher: AiByokMetadataCipher; protectedOrganizationId: string; quota?: AiCreditQuotaPort; meter?: AiCreditMeterPort; audit?: AiCreditAuditPort; now?: () => Date }>) {
  ...
  return Object.freeze({ repository, service, scopedRoutes: createAiCreditScopedRouteDeclarations({ service, repository }), jobs: aiCreditExpiryJobRegistration({ service, protectedOrganizationId: input.protectedOrganizationId, now: input.now }), transferRepository, transferStep: createAiByokTransferStep(transferRepository) })
}
