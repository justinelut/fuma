...
function assertGrant(input: FumaScopedRouteHandlerInput, connectorCapabilities: readonly string[], toolCapabilities: readonly CoreCapability[]): void { const held = new Set(input.context.capabilities); if (toolCapabilities.some((capability) => !held.has(capability))) throw new McpAuthorityError('denied', 'Connector cannot exceed the current actor tool capabilities.'); if (connectorCapabilities.includes('site.publish') && (!held.has('ai.tools.write') || !held.has('pages.publish'))) throw new McpAuthorityError('denied', 'Publish connectors require current write and publish authority.'); if (connectorCapabilities.includes('site.mutate') && !held.has('ai.tools.write')) throw new McpAuthorityError('denied', 'Mutation connectors require current write authority.') }
...
export function createMcpScopedRouteDeclarations(service: McpService, confirmations?: McpPublishConfirmationIssuerPort): readonly FumaScopedRouteDeclaration[] {
  ...
  return Object.freeze([
    ...
    ...(confirmations ? [route('POST', '/ai/mcp/connectors/:connectorId/publish-confirmations', 'pages.publish', async (input) => {
      ...
      if (connector.state !== 'active' || !connector.capabilities.includes('site.publish') || Date.parse(connector.expiresAt) <= Date.now()) throw new McpAuthorityError('denied', 'An active publish connector is required.')
