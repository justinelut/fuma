...
const WEB_SERVER_PUBLIC_ORIGINS = [process.env.PUBLIC_ORIGIN, ADMIN_ORIGIN]
  .filter((value): value is string => typeof value === 'string' && value.length > 0)
  .join(',')
const E2E_VITE_MODE = process.env.E2E_VITE_MODE === 'preview' ? 'preview' : 'serve'
const WEB_SERVER_READY_URL = E2E_VITE_MODE === 'preview'
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      E2E_PUBLIC_BASE_URL: PUBLIC_BASE_URL,
      PUBLIC_ORIGIN: WEB_SERVER_PUBLIC_ORIGINS,
      E2E_VITE_MODE,
    },
