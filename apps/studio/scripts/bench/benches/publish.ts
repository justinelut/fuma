...
async function seedDraftSite(api: ServerApi, db: Db, pageCount: number): Promise<void> {
  ...
  `
  await api.saveDraftSite(db, SELF_HOST_SITE_ID, makeShell(api))
    ...
    try {
      ...
      if (published) {
        try {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            await api.getSetupStatus(published.db, SELF_HOST_SITE_ID)
          }
          const samples: number[] = []
          for (let i = 0; i < iters; i++) {
            const t0 = performance.now()
      ...
      const saveRows: BenchRow[] = []
      {
        ...
        try {
          ...
          await api.saveDraftSite(fresh.db, SELF_HOST_SITE_ID, shell)
