...
async function bundleEntry(entry: string): Promise<string> {
  ...
  // Bun emits source-label comments relative to process.cwd(); full tests run
  // from the workspace root while bootstrap:sync runs from apps/studio.
  return bundled.replace(/^(\s*\/\/ )apps\/studio\//gm, '$1')
}
