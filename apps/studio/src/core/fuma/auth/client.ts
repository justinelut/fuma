...
export async function beginHostedStaffGoogleSignIn(
  ...
  if (callback.pathname !== '/admin' || callback.search || callback.hash) {
    throw new Error('Staff Google callback must target the admin root')
  }
  return new URL('/api/auth/central-google', callback.origin).toString()
}
