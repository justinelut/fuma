...
if (!FormatRegistry.Has('date-time')) {
  FormatRegistry.Set('date-time', (value) => RFC3339_DATE_TIME.test(value) && Number.isFinite(Date.parse(value)))
}
if (!FormatRegistry.Has('email')) {
  FormatRegistry.Set('email', (value) => value.length <= 320 && EMAIL_ADDRESS.test(value))
}
