...
const BASE_MODULE_INDEX = join(STUDIO_ROOT, 'src/modules/base/index.ts')
...
describe('base module registration', () => {
  it('only imports available production base modules', async () => {
    const baseIndex = await Bun.file(BASE_MODULE_INDEX).text()
  ...
  it('does not keep retired module directories around', () => {
    for (const retiredPath of [
      ...
      expect(existsSync(join(STUDIO_ROOT, retiredPath))).toBe(false)
    }
  })
