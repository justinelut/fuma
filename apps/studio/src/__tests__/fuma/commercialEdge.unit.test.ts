...
describe('FUMA-048–062 commercial edge unit behavior',()=>{
 ...
 it('FUMA-050 normalizes case, port and trailing dot and rejects reserved or IDN labels',()=>{expect(normalizePublicHost('Tenant.Fuma.Co.Ke:443.')).toBe('tenant.fuma.co.ke');expect(normalizeFreeHostLabel('safe-tenant')).toBe('safe-tenant');expect(()=>normalizeFreeHostLabel('admin')).toThrow(FreeHostError);expect(()=>normalizeFreeHostLabel('ténant')).toThrow(FreeHostError)})
 it('FUMA-051 deletes only host-separated cache prefixes',async()=>{const cache=new MemoryEdgeCache();await cache.put('edge:a.fuma.co.ke:site:r:/','' as never);await cache.put('edge:b.fuma.co.ke:site:r:/','' as never);expect(await cache.deletePrefix('edge:a.fuma.co.ke:')).toBe(1);expect(cache.entries.has('edge:b.fuma.co.ke:site:r:/')).toBe(true)})
