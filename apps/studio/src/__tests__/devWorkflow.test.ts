...
function readStudioFile(path: string) {
  return readFileSync(new URL(path, studioRoot), 'utf-8')
}

function readWorkspaceFile(path: string) {
  return readFileSync(new URL(path, workspaceRoot), 'utf-8')
}
...
describe('development workflow', () => {
  it('`bun run dev` is the one-command launcher for cms + vite', () => {
    ...
    expect(existsSync(new URL('scripts/dev.ts', studioRoot))).toBe(true)
    expect(existsSync(new URL('scripts/vite.ts', studioRoot))).toBe(true)
    expect(existsSync(new URL('scripts/dev-all.ts', studioRoot))).toBe(false)

    const script = readStudioFile('scripts/dev.ts')
    ...
    // Manages the workspace-root docker postgres + app containers.
    expect(script).toContain("resolve(import.meta.dir, '../../..')")
    expect(script).toContain("join(WORKSPACE_ROOT, 'docker-compose.yml')")
    expect(script).toContain("'--project-directory', WORKSPACE_ROOT")
  ...
  it('development launchers route local Vite binaries through Bun on Windows', () => {
    const devScript = readStudioFile('scripts/dev.ts')
    const e2eScript = readStudioFile('scripts/e2e-dev.ts')
    const startScript = readStudioFile('scripts/start.ts')
    const viteScript = readStudioFile('scripts/vite.ts')
  ...
  it('Vite proxies CMS API and uploaded media to the local Bun server', () => {
    const viteConfig = readStudioFile('vite.config.ts')
