...
const GRANDFATHERED: Record<string, number> = {
  // Fuma tenant-object receipts are an exhaustive immutable evidence codec;
  // keep it under an exact ratchet while the receipt vocabulary remains unified.
  'server/fuma/tenantObjects/receipts.ts': 736,
  // The layered permission resolver keeps one closed decision table so precedence
  // and protected-owner invariants cannot drift across multiple implementations.
  'server/fuma/permissions/resolver.ts': 761,
  // Transfer persistence is one atomic aggregate repository with lock/fence
  // mapping kept together; this exact cap prevents further growth.
  'server/fuma/transfers/repository.ts': 992,
  // Transfer contracts are the TypeBox authority for the complete saga protocol.
  'server/fuma/transfers/contracts.ts': 781,
  // Durable transfer jobs intentionally centralize forward/compensation fencing
  // and idempotency; this known hotspot is frozen at its introduction size.
  'server/fuma/transfers/jobs.ts': 1224,
  // Transfer orchestration keeps proposal/confirm/start authorization in one
  // aggregate service; additions must first extract existing responsibilities.
  'server/fuma/transfers/service.ts': 1001,
  // Hosted migration 000008 is immutable additive SQL and its rollback/evidence
  // declarations must remain byte-stable after release; freeze its source size.
  'server/fuma/db/migrations/000008_transfer_saga.ts': 918,
  // Fuma dispatch expanded the canonical server router; keep the current facade
  // on an exact ratchet until route-family extraction brings it below CEILING.
  'server/router.ts': 763,
