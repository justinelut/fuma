 ...
 *
 * Current Instatic runtime files under `server/` that import `DbClient` may not
 * use PostgreSQL-specific SQL syntax or types. The separately governed
 * `server/fuma/` hosted runtime is PostgreSQL-only and is excluded as a domain,
 * rather than by an expanding per-file allowlist. The historical dialect
 * migrations and dialect-aware JSON helper remain explicit exceptions.
 *
 ...
 * Scanning all of `server/` keeps layout coverage broad; the file filter below
 * then excludes the separately governed PostgreSQL-only Fuma hosted domain and
 * limits findings to query-issuing legacy runtime files.
 */
...
const HOSTED_POSTGRES_ROOT = join(PROJECT_ROOT, 'server/fuma')

function isDialectPortableRuntime(file: string): boolean {
  return (
    file !== HOSTED_POSTGRES_ROOT &&
    !file.startsWith(`${HOSTED_POSTGRES_ROOT}/`)
  )
}
...
function scanForViolations(): Violation[] {
  const files = SCAN_ROOTS.flatMap((root) => walk(root)).filter(
    (file) => isDialectPortableRuntime(file) && !ALLOWLISTED.has(file),
  )
...
describe('Postgres-ism isolation — DbClient-consuming server files', () => {
  ...
  test('keeps hosted Fuma PostgreSQL-only and legacy runtime dialect-portable', () => {
    expect(
      isDialectPortableRuntime(
        join(PROJECT_ROOT, 'server/fuma/jobs/repository.ts'),
      ),
    ).toBeFalse()
    expect(
      isDialectPortableRuntime(
        join(PROJECT_ROOT, 'server/repositories/site.ts'),
      ),
    ).toBeTrue()
  })
