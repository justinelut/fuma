...
describe('Toolbar — structural requirements', () => {
  ...
  it('AdminCanvasLayout imports and renders Toolbar', () => {
    ...
    expect(src).toContain('const persistence = usePersistence(cmsAdapter, {')
    expect(src).toContain('markNewSiteUnsaved: true')
    expect(src).not.toContain("usePersistence('default'")
    expect(src).not.toContain("loadSite('default'")
