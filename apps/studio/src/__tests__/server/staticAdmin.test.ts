...
describe('self-hosted admin static serving', () => {
  ...
  it('selects hosted auth shells without starting self-hosted boot requests', async () => {
    const staticDir = createAdminShellFixture()
    const hostedCookieName = '__Host-fuma_staff'
    try {
      ...
      expect(loginHtml).toContain('window.__fumaHostedStaffAuth = 1')
      expect(loginHtml).toContain('data-initial-login-skeleton="true"')
      ...
      expect(authenticatedHtml).toContain('window.__fumaHostedStaffAuth = 1')
      expect(authenticatedHtml).toContain('window.__instaticAuthed = 1')
      expect(authenticatedHtml).toContain('AuthenticatedAdmin-test.js')
      expect(authenticatedHtml).not.toContain('window.__instaticBootPromises')
    } finally {
      rmSync(staticDir, { recursive: true, force: true })
    }
  })
