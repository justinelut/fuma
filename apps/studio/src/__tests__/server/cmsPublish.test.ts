...
function createPublishFakeDb() {
  ...
  const db = createFakeDb(async (rawSql, params): Promise<DbResult> => {
    ...
    // saveDraftSite — insert or update explicitly selected site row (NOT site_snapshots)
    ...
    // getDraftSite — select the explicitly requested site row
    if (sql.startsWith('select id, name, settings_json')) {
      const matches = state.site?.id === params[0]
      return { rows: matches ? [state.site] : [], rowCount: matches ? 1 : 0 }
    }
...
async function seedSiteAndPage(
  ...
  await saveDraftSite(db, SELF_HOST_SITE_ID, shell)
