...
function makeFakeDb() {
  ...
  const handle = async <Row = Record<string, unknown>>(
    ...
    // getSetupStatus — values[0]=siteId
    if (normalized.includes('count(*) as count') && normalized.includes('from site')) {
      return { rows: [{ count: site.filter((row) => row.id === values[0]).length } as Row], rowCount: 1 }
    }
    ...
    // createSite / saveDraftSite — values[0]=siteId, values[1]=name, values[2]=settings
    if (normalized.includes('insert into site')) {
      const row = { id: values[0], name: values[1], settings_json: values[2] }
      const index = site.findIndex((candidate) => candidate.id === values[0])
