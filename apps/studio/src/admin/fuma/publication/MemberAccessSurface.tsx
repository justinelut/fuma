...
import styles from './MemberAccessSurface.module.css'
...
export function MemberAccessSurface({ client, canWrite, members }: Props) {
  ...
  return <div className={styles.layout}>
    <section className={styles.panel} aria-labelledby="member-profile-title">
    ...
    <section className={styles.panel} aria-labelledby="member-segments-title">
      ...
      <ul className={styles.rows}>{segments.map((segment) => <li key={segment.segmentId}><span><strong>{segment.name}</strong><small>{segment.kind} · v{segment.version} · {segment.recalculatedAt ? new Date(segment.recalculatedAt).toLocaleString() : 'not calculated'}</small></span><Button variant="ghost" size="sm" type="button" disabled={!canWrite || busy} onClick={() => void recalculate(segment.segmentId)}>Recalculate</Button></li>)}</ul>
      ...
      {segmentKind === 'dynamic' ? <div className={styles.fields}><FormField label="Rule field" htmlFor="member-segment-field"><Input id="member-segment-field" value={segmentField} onChange={(event) => setSegmentField(event.target.value)} disabled={!canWrite || busy}/></FormField><FormField label="Rule value" htmlFor="member-segment-value"><Input id="member-segment-value" value={segmentValue} onChange={(event) => setSegmentValue(event.target.value)} disabled={!canWrite || busy}/></FormField></div> : <p className={styles.meta}>The selected member is the explicit membership source.</p>}
    ...
    <section className={styles.panel} aria-labelledby="member-access-title">
      ...
      <div className={styles.tableWrap}><table><caption>Access for {selectedMember?.name || selectedMember?.email || 'selected member'}</caption><thead><tr><th>Source</th><th>Resource</th><th>State</th><th>Expiry</th><th><span className={styles.srOnly}>Actions</span></th></tr></thead><tbody>{access.map((value) => <tr key={value.accessId}><td>{value.source}</td><td>{value.resourceKind}: {value.resourceId}</td><td>{value.state}</td><td>{value.expiresAt ? new Date(value.expiresAt).toLocaleDateString() : 'Never'}</td><td><Button variant="ghost" size="sm" type="button" disabled={!canWrite || busy || value.state === 'revoked'} onClick={() => void revoke(value)}>Revoke</Button></td></tr>)}</tbody></table></div>
      <div className={styles.fields}><FormField label="Source" htmlFor="member-access-source"><Select id="member-access-source" value={source} onChange={(event) => setSource(event.target.value as typeof source)} disabled={!canWrite || busy} options={[{ value: 'complimentary', label: 'Complimentary' }, { value: 'manual', label: 'Manual' }, { value: 'paid', label: 'Paid (verified digest)' }]}/></FormField><FormField label="Access" htmlFor="member-access-kind"><Select id="member-access-kind" value={accessKind} onChange={(event) => setAccessKind(event.target.value as typeof accessKind)} disabled={!canWrite || busy} options={[{ value: 'read', label: 'Read' }, { value: 'premium', label: 'Premium' }]}/></FormField></div>
      <div className={styles.fields}><FormField label="Resource kind" htmlFor="member-resource-kind"><Select id="member-resource-kind" value={resourceKind} onChange={(event) => setResourceKind(event.target.value as typeof resourceKind)} disabled={!canWrite || busy} options={[{ value: 'publication', label: 'Publication' }, { value: 'post', label: 'Post' }, { value: 'tag', label: 'Tag' }]}/></FormField><FormField label="Resource ID" htmlFor="member-resource-id"><Input id="member-resource-id" value={resourceId} onChange={(event) => setResourceId(event.target.value)} disabled={!canWrite || busy}/></FormField></div>
      <div className={styles.fields}><FormField label="Expires at" htmlFor="member-access-expiry"><Input id="member-access-expiry" type="datetime-local" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} disabled={!canWrite || busy}/></FormField><FormField label="Grace ends at" htmlFor="member-access-grace"><Input id="member-access-grace" type="datetime-local" value={graceEndsAt} onChange={(event) => setGraceEndsAt(event.target.value)} disabled={!canWrite || busy}/></FormField></div>
      {source === 'paid' ? <FormField label="Verified payment reference SHA-256" htmlFor="member-payment-digest"><Input id="member-payment-digest" monospace value={paymentDigest} minLength={64} maxLength={64} onChange={(event) => setPaymentDigest(event.target.value)} disabled={!canWrite || busy}/></FormField> : null}
    ...
    <section className={styles.panel} aria-labelledby="member-privacy-title">
      ...
      {privacyRequest ? <p className={styles.meta}>Request {privacyRequest.requestId} · {privacyRequest.state}</p> : null}
    </section>
