...
export function McpScopedConnectorPanel({ siteId, connectors, onCreate, onRevoke }: Readonly<{ siteId: string; connectors: readonly ScopedMcpConnectorView[]; onCreate(intent: ScopedMcpCreateIntent): Promise<Readonly<{ token: string }>>; onRevoke(connectorId: string): Promise<void> }>) {
  ...
  return <section className={styles.root} aria-labelledby="scoped-mcp-title">
    <header className={styles.header}><div><h2 id="scoped-mcp-title">Site MCP connectors</h2><p>Tokens are site-bound, shown once, hashed at rest, expiring, and immediately revocable.</p></div><span className={styles.siteBadge}>{siteId}</span></header>
    <form className={styles.form} onSubmit={(event) => void submit(event)}><label className={styles.field}><span>Connector label</span><Input value={label} onChange={(event) => setLabel(event.currentTarget.value)} required placeholder="Deployment assistant" /></label><fieldset className={styles.capabilities}><legend>Exact capabilities</legend>{OPTIONS.map((option) => <label key={option.id} className={styles.option}><input type="checkbox" checked={selected.has(option.id)} onChange={() => setSelected((current) => { const next = new Set(current); if (next.has(option.id)) next.delete(option.id); else next.add(option.id); return next })} /><span><strong>{option.label}</strong><small>{option.detail}</small></span></label>)}</fieldset>{error && <p role="alert" className={styles.error}>{error}</p>}<Button type="submit" variant="primary" size="sm" disabled={busy || !label.trim() || selected.size === 0}><span>{busy ? 'Creating…' : 'Create scoped connector'}</span></Button></form>
    {createdToken && <div className={styles.token} role="status"><strong>Copy this token now. It will not be shown again.</strong><code>{createdToken}</code><Button type="button" variant="secondary" size="sm" onClick={() => void navigator.clipboard.writeText(createdToken)}><span>Copy token</span></Button></div>}
    <div className={styles.list}>{connectors.length === 0 ? <p className={styles.empty}>No connector is bound to this site.</p> : connectors.map((connector) => <article className={styles.card} key={connector.connectorId}><div><strong>{connector.label}</strong><p>{connector.capabilities.join(' · ')} · generation {connector.ownerGeneration}</p><small>{connector.state === 'active' ? `Expires ${new Date(connector.expiresAt).toLocaleDateString()}` : connector.state}</small></div><Button type="button" variant="secondary" size="sm" disabled={connector.state !== 'active'} onClick={() => void onRevoke(connector.connectorId)}><span>{connector.state === 'revoked' ? 'Revoked' : 'Revoke'}</span></Button></article>)}</div>
  </section>
}
