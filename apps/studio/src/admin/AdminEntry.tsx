...
// Hosted Fuma surfaces must not inflate the self-hosted login entry. They are
// selected only when hosted auth is enabled, so keep each behind its own chunk.
const HostedStaffPreAuth = prewarmedLazy(
...
export interface AdminEntryProps {
  ...
  /**
   * FUMA-021 replaces this test/composition seam with trusted server context.
...
export default function AdminEntry(props: AdminEntryProps) {
  return hostedStaffAuthSelected()
    ? <HostedAdminEntry {...props} />
    : <SelfHostedAdminEntry {...props} />
}
