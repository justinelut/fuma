...
| ID | Priority | Auto | Area | User Goal | Setup | Path | Expected Outcome | Watch For |
|---|---:|:---:|---|---|---|---|---|---|
| CONFIG-001 | P1 | partial | Database | Run the CMS on PostgreSQL in local, test, and production environments | PostgreSQL DATABASE_URL set | Server boot / DB helper | PostgreSQL schemes select the supported adapter, migrations run idempotently, JSON columns follow naming rules, and queries bind values | unsupported URLs, connection failure, migration drift, JSON column suffix drift, advisory-lock failures |
...
CONFIG-001 note: PostgreSQL URL validation, migration idempotence, JSON-column naming, bound-query behavior, row counts, transactions, advisory locks, statement caching, development workflow, and production configuration form the supported database contract. Live PostgreSQL failure/recovery remains operator-run.
...
## Hosted Fuma Publication

| ID | Priority | Auto | Area | User Goal | Setup | Path | Expected Outcome | Watch For |
|---|---:|:---:|---|---|---|---|---|---|
| FUMA-PUBLICATION-028-047 | P0 | partial | Hosted Publication | Collaborate on scoped editorial content, manage readers/newsletters, and deliver an immutable campaign | Hosted Publication site with exact permission decisions and OCI Email Delivery configured | `https://5174.blyss.co.ke/admin/organizations/<organization>/workspaces/<workspace>/sites/<site>/newsletters` plus `https://3002.blyss.co.ke` | Ordered capability-owned workspaces hydrate; denied routes mount no child; complete ancestry is retained; previews use the FUMA-042 data-only renderer; campaign delivery remains OCI-only | profile branches, caller tenant authority, draft overwrite, mutable snapshots, suppression races, localhost acceptance |

FUMA-PUBLICATION-028-047 note: `tests/e2e/fuma-publication-phase.spec.ts` covers the Blyss HTTPS shell/public hosts, ordered Publication navigation, denied-child mount behavior, and complete-ancestry API paths. Deterministic focused suites own collaboration, workflow, audience, settings, rendering, campaign, suppression, unsubscribe, provider-event, PostgreSQL, fault, and security behavior; this coding-only phase records no executed acceptance evidence.
