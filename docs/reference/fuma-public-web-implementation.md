...
The approved final visual and animation direction is [`fuma-public-web-visual-motion-direction.md`](fuma-public-web-visual-motion-direction.md). FUMA-WEB-012 now owns its original Fuma hero-to-footer implementation and exact-pinned `motion@12.43.0` integration through a narrow `motion/react` `LazyMotion` product-canvas island; FUMA-WEB-018 owns the later complete Blyss-only launch visual matrix and sign-off. The single root lock update is explicit and no second animation runtime is present.

## Runtime boundaries

Public browsers use only `trimly.co.ke` or the required acceptance endpoint `https://3002.blyss.co.ke`. `/api/public/v1/*`, `/api/handoff`, `/api/contact`, `/api/events`, and `/api/vitals` require exact HTTPS Host/Origin agreement where applicable, bounded JSON, and strict TypeBox values, then construct private requests from scratch. Every submission response is no-store and emits no cookie. Browser cookies and authorization headers are never forwarded. The private origin, projection token, preview token, and metrics token are server-only configuration. There is no public API hostname.

The public app has no identity session. `/start` validates a closed `PublicHandoffRequest`, posts it to the same-origin handoff route, and receives only an app-host URL containing a short-lived opaque intent and correlation. Product-owned issuance, centralized authentication, single-use auth-code exchange, cancellation, app-host session establishment, and fresh authority re-resolution are composed by FUMA-WEB-013; see [`fuma-public-handoff.md`](fuma-public-handoff.md).
...
These results satisfy repository-achievable FUMA-WEB-007 acceptance. Central app intent issuance/consumption and sessions were intentionally still open at FUMA-WEB-007 acceptance and are now composed by FUMA-WEB-013; production DNS/TLS, low-end field-vital sign-off, legal approval, and unified launch signatures remain later launch-gate evidence rather than WEB-007 claims.
...
## FUMA-WEB-013 secure handoff composition (2026-07-30)

Studio now mounts the issuer inside the same exact private Web service boundary and owns separate exact-host app/auth resume surfaces. PostgreSQL migration `000077_public_handoff_authority`, checksum `fb257b84c2e44b65212ef5227845c50524887248fb10732a6ec46a4b7dd53015`, stores only SHA-256 bearer digests, fixes the `fuma-app` audience and `/resume` callback, serializes one-time intent/code consumption, binds each code and app-host relying session to the exact current Better Auth session, and retains non-PII append-only transition evidence. Central auth exposes no admin endpoint through this surface. App confirmation and cancellation are same-origin and fixed-target; exchange freshly re-resolves profile, current effective plan/version, retained approved template release, or currently approved mediated expert authority. Full threat model and route details: [`fuma-public-handoff.md`](fuma-public-handoff.md).
