...
- 132 gate files across structural domains: SQL, JSON columns, migrations, CSS, icons, primitives, page tree, sandbox, agent, router, content storage, boundary validation, module size, AI, auth, error handling, etc.
...
| `fuma-metering.test.ts`                       | FUMA-052 metering composition: complete nonzero cost baseline seeding, trusted reconciliation job registration, replay-safe publish/newsletter producer wrapping, stable physical usage evidence, and no authority-bearing durable payloads. |
| `fuma-entitlements.test.ts`                   | FUMA-054 entitlement boundary: strict TypeBox contracts, explicit public/private pricing split, protected internal grant, PostgreSQL runtime authority, awaiting-payment-only atomic acceptance, unregistered additive evidence migration, exact lifecycle transitions, and bounded modules. |
| `fuma-public-templates.test.ts`               | FUMA-WEB-010 template authority: PostgreSQL approval/withdrawal composition, deletion-proof immutable release evidence, exact preview-host dispatch before tenant routing, approved image MIME limits, stable-ID installation handoff, and discovery sitemap eligibility. |
