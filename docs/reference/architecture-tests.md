...
| `fuma-entitlements.test.ts`                   | FUMA-054 entitlement boundary: strict TypeBox contracts, explicit public/private pricing split, protected internal grant, PostgreSQL runtime authority, awaiting-payment-only atomic acceptance, finalized additive evidence migration, exact lifecycle transitions, and bounded modules. |
