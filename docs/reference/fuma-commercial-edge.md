...
- **050 Free hosts:** [`freeHosts/`](fuma-free-hosts.md) durably allocates normalized, IDN-safe, reserved-safe `<tenant>.trimly.co.ke`; the production Host boundary resolves exact active immutable releases, and the deterministic demo serves two exact hosts with no fallback.
...
- **054 Plans/entitlements:** [`entitlements/`](../../apps/studio/server/fuma/entitlements) requires strict finite quotas, exact KES monthly/annual pairs, complete 18-meter invoice/quote/baseline evidence, >=70% margin, <=30% variable COGS, separately covered setup cost, one protected non-transferable internal grant, immutable versioned offers, and atomic awaiting-payment candidates. Acceptance never activates or hands off. See [platform plans and entitlements](fuma-platform-entitlements.md).
