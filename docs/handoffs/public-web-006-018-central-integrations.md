...
1. **Studio authority composition — completed by FUMA-WEB-006/FUMA-WEB-010 (2026-07-27):** hosted startup registers strict server-owned `PublicProjectionAuthority` adapters for `product-facts`, `pricing`, `templates`, `showcases`, `experts`, and `plugins`. Templates now read only approved immutable PostgreSQL releases through the release authority, produce deletion-proof withdrawal tombstones, and expose exact isolated preview artifacts; the earlier fail-empty placeholder is removed. IDs are display-safe, versions are content-addressed, cursors are dataset-bound, and all pages pass the shared strict contracts.
...
3. **Contact route — private composition complete, production authority pending:** `POST /_fuma/private/public/v1/contact` now shares the strict TypeBox contract, authenticates the Web audience, rejects visitor credentials, applies a central Redis aggregate limit, and forwards only to an explicitly configured private HTTPS sink with bounded timeout/no redirects. Cross-replica durable replay state, approved delivery routing/retention/escalation, and production sink evidence remain required.
...
Identity/app resume endpoints, contact delivery approval/durable cross-replica replay, analytics storage/dashboard/retention jobs, moderation and pricing purge-event wiring owned by FUMA-WEB-011/FUMA-WEB-009, edge/DNS/TLS accounts, image registry digests, monitoring backend, legal approval, production status authority, and named production owners remain outside this phase. Public pages continue to fail closed when an approved dataset is empty, withdrawn, malformed, or unavailable.
