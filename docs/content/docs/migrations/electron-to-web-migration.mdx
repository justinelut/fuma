...
### Extra stuff
```
Local memory 	->	Hosted PostgreSQL + caching
```
Previously, we stored most user and project information on the user's machine as serialized JSON. The web application persists it in hosted PostgreSQL tables accessed through Drizzle, with a browser caching layer for responsive loading.
