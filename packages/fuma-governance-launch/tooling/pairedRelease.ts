...
export const RuntimeSmokeSchema = Type.Object({
  ...
  roles: Type.Array(Type.Object({
    ...
    logsSha256: Type.String({ pattern: '^[a-f0-9]{64}$' }),
  ...
  emailRenderer: Type.Object({
    ...
    htmlSha256: Type.String({ pattern: '^[a-f0-9]{64}$' }),
...
type SmokeSchema = typeof RuntimeSmokeSchema | typeof WebSmokeSchema
type RuntimeSmoke = Static<typeof RuntimeSmokeSchema>

function assertRuntimeSmokeSemantics(value: RuntimeSmoke, expectedPlatform: string): void {
  ...
  const expectedArch = expectedPlatform === 'linux/amd64' ? 'x64' : 'arm64'
  if (value.emailRenderer.arch !== expectedArch) throw new Error(`Email renderer architecture must be ${expectedArch}`)
}
...
export async function evidenceDirectoryHash(
  ...
  const expectedNames = ['amd64.json', 'arm64.json']
  const names = entries.map(({ name }) => name).sort()
  if (names.join(',') !== expectedNames.join(',')) {
    throw new Error(`${basename(directory)} must contain only amd64.json and arm64.json`)
  }
  if (entries.some((entry) => !entry.isFile())) throw new Error(`${basename(directory)} smoke records must be regular files`)

  const evidence = await Promise.all(expectedNames.map(async (name) => {
    ...
    const expectedPlatform = name === 'amd64.json' ? 'linux/amd64' : 'linux/arm64'
    if (value.sourceSha !== sourceSha) throw new Error(`Mixed source SHA in ${name}`)
    if (value.lockHashSha256 !== lockHashSha256) throw new Error(`Lock hash mismatch in ${name}`)
    if (value.migrationHighWaterMark !== migrationHighWaterMark) throw new Error(`Migration high-water mismatch in ${name}`)
    if (value.platform !== expectedPlatform) throw new Error(`Platform mismatch in ${name}`)
    if (value.image !== expectedImage) throw new Error(`Smoke image mismatch in ${name}`)
    if (schema === RuntimeSmokeSchema) assertRuntimeSmokeSemantics(value as RuntimeSmoke, expectedPlatform)
    return { name, bytes }
  }))
  return hashPairedReleaseFile(Buffer.concat(evidence.flatMap(({ name, bytes }) => [Buffer.from(`${name}\0`), bytes])))
}
...
async function verifyMode(input: ReadonlyMap<string, string>): Promise<void> {
  ...
  if (!match || match[1] !== hashPairedReleaseFile(bytes) || match[2] !== basename(manifestPath)) {
    throw new Error('Paired release file digest mismatch')
  }
  ...
  if (evidenceMode === 'full') {
    ...
    const checks = await Promise.all([
      ...
      validatedPublicationPlanHash(required(input, 'publication-plan'), manifest.sourceSha, manifest.runtimeImage, manifest.webImage),
    ])
    const expected = [
...
async function createMode(input: ReadonlyMap<string, string>): Promise<void> {
  ...
  if (rootPackage.packageManager !== 'bun@1.3.14'
  ...
  const manifest = createPairedReleaseManifest({
    ...
    publicationPlanHashSha256: await validatedPublicationPlanHash(required(input, 'publication-plan'), sourceSha, runtimeImage, webImage),
  })
